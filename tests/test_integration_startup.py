...
class TestFetchAndRunTwinDriversIntegration:
    ...
    def test_macos_passes_camera_bridge_candidates_to_driver_launcher(self, tmp_path, monkeypatch):
        self._patch_common(tmp_path, monkeypatch)
        twin = _make_fake_twin(driver_image="cyberwaveos/camera-driver")
        client = self._make_client([twin])
        ...
        monkeypatch.setattr(
            startup,
            "_resolve_macos_camera_bridge_candidates",
            lambda asset, twin_metadata: ["/dev/video0"],
        )
