...
## Suggested TF implementation proposal (not implemented yet)

This proposal is intentionally TF-focused and based on the current stack (`edge-runtime` service layers, existing ROS bridge drivers, cloud-node MQTT runtime, and SDK MQTT conventions).

### 1) Core idea: split TF into 2 explicit layers

#### Layer A: In-driver TF adapter (ROS or native)

Each driver uses a common TF interface and chooses the adapter at startup:

- `RosTfAdapter`: wraps ROS 2 tf2 (`/tf`, `/tf_static`, `tf2_ros::Buffer`/listener/broadcaster)
- `NativeTfAdapter`: non-ROS tf2-like implementation (pure C++ or Rust, no ROS dependency)

Driver code only sees `ITfAdapter`, never direct ROS tf2 calls.

#### Layer B: TF exchange container (cross-container + cloud bridge)

A dedicated container runs independently of drivers and is protocol-agnostic at API level:

- receives transform updates from all drivers
- maintains canonical edge-local TF graph
- serves lookup requests for cross-container use cases
- republishes to cloud at a lower, configurable rate
- imports selected TF from cloud back to edge

This isolates inter-container coordination from driver internals.

### 2) Proposed concrete locations in this repo

- Driver shared TF interface:
  - `cyberwave-edge-runtime/runtime-services/drivers/common/python/tf/`
  - `cyberwave-edge-runtime/runtime-services/drivers/common/cpp/tf/`
- TF exchange container (new runtime service):
  - `cyberwave-edge-runtime/runtime-services/libraries/tf-exchange/`
- Cloud counterpart (optional edge/cloud exchange policy logic):
  - `cyberwave-cloud-nodes/cyberwave-cloud-node/cyberwave_cloud_node/tf_exchange/`
- Shared SDK contracts:
  - `cyberwave-sdks/cyberwave-python/mqtt/tf/`
  - `cyberwave-sdks/cyberwave-cpp/mqtt/tf/`

### 3) Driver-facing interface contract

Define one interface that both ROS and native driver bases can initialize:

```text
ITfAdapter
  - start()
  - publish_dynamic(transform)
  - publish_static(transform)
  - lookup(target_frame, source_frame, timestamp_ns, timeout_ms)
  - can_transform(target_frame, source_frame, timestamp_ns, timeout_ms)
  - close()
```

Adapter selection in driver base:

- `TF_BACKEND=ros` -> `RosTfAdapter`
- `TF_BACKEND=native` -> `NativeTfAdapter`

Optional exchange binding mode:

- `TF_EXCHANGE_MODE=push` (driver publishes only)
- `TF_EXCHANGE_MODE=push_pull` (driver publishes + delegates lookup to exchange)

### 4) TF exchange container responsibilities

The `tf-exchange` service is the canonical TF coordinator across containers:

1. Ingest transforms from driver containers
2. Apply authority policy (single writer per edge)
3. Maintain dynamic + static graph in memory
4. Serve lookup/can_transform API for local clients
5. Replicate to cloud with configurable decimation/throttling
6. Import approved remote TF streams into a dedicated namespace

Important: control loops must keep local behavior if exchange-to-cloud path is unavailable.

### 5) Data contract (transport-neutral payload)

Use one normalized TF message independent from ROS types:

```json
{
  "robot_id": "robot_1",
  "authority": "odometry",
  "parent_frame": "robot_1/odom",
  "child_frame": "robot_1/base_link",
...
- one writer authority per `(parent_frame, child_frame)`
- monotonic `sequence` per authority edge
- mandatory robot (TWIN_UUID) namespacing for all frames

### 6) Container-to-container transport choice in this stack

Use existing local MQTT pattern first (lowest integration risk with current drivers):

- Driver -> tf-exchange ingest topics:
  - `cyberwave/local/tf/{robot_id}/dynamic`
  - `cyberwave/local/tf/{robot_id}/static`
- tf-exchange -> local query API:
  - lightweight gRPC or HTTP in-container endpoint
- tf-exchange -> Cloud replication topics (decimated):
  - `cyberwave/tf/{robot_id}/dynamic`
  - `cyberwave/tf/{robot_id}/static`
- Cloud -> tf-exchange import topics:
  - `cyberwave/tf-import/{robot_id}/#`

Phase 2 can add Zenoh path in parallel, but MQTT gives immediate compatibility with current runtime and cloud-node conventions.

### 7) Cloud synchronization policy (slow, configurable, safe)

Default behavior:

- local graph updates at full driver rate (e.g. 30-100 Hz)
- cloud publish at configurable lower rate (e.g. 1-5 Hz for dynamic, immediate for static changes)

Recommended config:

- `TF_CLOUD_DYNAMIC_RATE_HZ=2`
- `TF_CLOUD_MAX_BURST=20`
- `TF_IMPORT_ENABLED=false` (default off)
- `TF_IMPORT_ALLOWLIST=fleet_map,robot_2/map`

Remote TF import must never override local critical edges (`map->odom`, `odom->base_link`) unless explicitly allowed.

### 8) Adapter behavior details

#### `RosTfAdapter`

- reads ROS `/tf` and `/tf_static`
- converts ROS messages to normalized TF contract
- publishes to tf-exchange ingest
- for lookups:
  - local-first (`tf2 buffer`) when within same container
  - optional fallback to tf-exchange API for cross-container chain queries

#### `NativeTfAdapter` (To come at a later point in time)

- no ROS dependencies
- uses local in-process graph for low-latency same-container lookup
- publishes normalized updates to tf-exchange
- can query tf-exchange for non-local frame chains

### 9) Failure and consistency model

- If tf-exchange is down:
  - driver local adapter still works for container-local TF
  - cross-container lookups degrade gracefully (`can_transform=false`, explicit error)
- If cloud is down:
  - tf-exchange continues local operations
  - replication queue drops/backs off according to bounded buffer policy
- Conflict handling:
  - first writer wins by policy for an edge until lease expiry or explicit authority transfer

### 10) Concrete rollout plan

1. Create shared `ITfAdapter` contract in drivers common (Python + C++).
2. Implement `RosTfAdapter` (and later `NativeTfAdapter` with normalized payload output).
3. Introduce `tf-exchange` container in `runtime-services/libraries/`.
4. Enable ingest-only mode first (no remote import).
5. Add local lookup API and migrate one ROS driver.
6. Add cloud replication decimation controls.
7. Enable remote import behind feature flag for one use case.

### 11) Definition of done

- Driver bases initialize TF via adapter interface only
- Both ROS and native drivers can publish/lookup through same API
- Cross-container TF works through `tf-exchange` without ROS coupling
- Cloud TF replication is configurable and rate-limited
- Control loops remain local-first and cloud-independent
- Authority conflicts and namespace violations are detected and logged
