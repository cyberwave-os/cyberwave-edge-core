...
Interfaces with no IPv4 address bound are excluded from both the message and the change signature. They're nothing anyone can SSH to, and they churn (`docker0`/`veth*` as driver containers cycle, `usb0` as cables move), which would otherwise re-fire the fan-out on changes that tell an operator nothing. This also matches what the frontend renders — it filters on the same condition.
