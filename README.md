...
The systemd unit generated by `cyberwave edge install` configures `WatchdogSec=60` and `Type=notify`. Edge Core pings the systemd watchdog every reconcile cycle (~15 s). If the process hangs or dies, systemd restarts it automatically. The unit also uses `Restart=always` (not `on-failure`) so any exit — including OOM kills — triggers a restart.

#### Hardware watchdog (Raspberry Pi)

On Linux devices with `/dev/watchdog` (all Raspberry Pi models), Edge Core optionally drives the hardware watchdog timer. If the edge-core process stops pinging within 30 seconds, the SoC resets the board. This is a last-resort safety net for kernel-level hangs or total process death.
