...
On `systemd` deployments where Edge Core itself runs as root, every per-model staging directory, downloaded weight, sidecar `metadata.json`, and the top-level `manifest.json` is also chowned to the invoking host user at write time — not just at startup. This keeps the cache readable from inside the worker container (which launches with `--user $UID:$GID`); without it, `cw.models.load` crashes with `PermissionError: [Errno 13]` on `iterdir` when traversing a root-owned `cache_dir/{model_id}/`. The wiring lives in `ModelManager(owner_uid_gid=resolve_config_owner_uid_gid())`.
